...
import CCBP as gcv
from CCBP import data as gdata
from CCBP import utils as gutils
from CCBP.model_zoo import get_model
from CCBP.data import batchify
from CCBP.data.transforms.presets.rcnn import FasterRCNNDefaultTrainTransform
from CCBP.data.transforms.presets.rcnn import FasterRCNNDefaultValTransform
from CCBP.utils.metrics.voc_detection import VOC07MApMetric
from CCBP.utils.metrics.coco_detection import COCODetectionMetric
from CCBP.utils.metrics.accuracy import Accuracy
...
def get_dataset(dataset, args):
    if dataset.lower() == 'voc':
        train_dataset = gdata.VOCDetection(root="D:/VOCdevkit/",
            splits=[(2007, 'trainval'), (2012, 'trainval')])
        val_dataset = gdata.VOCDetection(root="D:/VOCdevkit/",
            splits=[(2007, 'test')])
        val_metric = VOC07MApMetric(iou_thresh=0.5, class_names=val_dataset.classes)
    elif dataset.lower() == 'coco':
        train_dataset = gdata.COCODetection(root='D:/coco/',splits='instances_train2017', use_crowd=False)
        val_dataset = gdata.COCODetection(root='D:/coco/',splits='instances_val2017', skip_empty=False)
        val_metric = COCODetectionMetric(val_dataset, args.save_prefix + '_eval', cleanup=True)
    else:
        raise NotImplementedError('Dataset: {} not implemented.'.format(dataset))
    if args.mixup:
        from CCBP.data.mixup.detection import MixupDetection
